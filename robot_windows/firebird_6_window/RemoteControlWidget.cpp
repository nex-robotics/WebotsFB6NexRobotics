...
#include "../../remote_controls/firebird_comm/UploaderData.hpp"
...
void RemoteControlWidget::cancelUpload() {
  UploaderData *cancelData = new UploaderData(UploaderData::CANCEL_CMD);
  callUploadCustomFunction(cancelData);
  delete cancelData;
}
...
void RemoteControlWidget::connectDevice() {
  ...
  delete cUploaderData;
  cUploaderData = new UploaderData(UploaderData::CONNECT_CMD, mPortName.toStdString());
...
void RemoteControlWidget::uploadController() {
  ...
  if (mHexFileName.isEmpty()) {
    // close connection
    UploaderData *disconnectData = new UploaderData(UploaderData::DISCONNECT_CMD);
  ...
  delete cUploaderData;
  cUploaderData = new UploaderData(UploaderData::SEND_CMD, mHexFileName.toStdString(), 100, RemoteControlWidget::updateProgress);
