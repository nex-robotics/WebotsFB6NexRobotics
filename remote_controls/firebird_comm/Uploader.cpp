...
static UploaderData::ProgressFunc gUpdateProgressFunction = NULL;
...
static int readfileINHX32(const std::string &filename) {
  ...
  while(getline(file, line)) {
    ...
    k = (line.length() - j) / 2 - 1;
    if (k != len) {
      fprintf(stderr, "wrong data lenght on line %d, ignored\n", line_counter);
      continue;
    }
    i += 2 * len;
    hex[0] = line[i++];
...
static int send_packets(Serial *serial) {
  ...
  for (j = 0; j < 100 ; j++) {
    ...
    for (int c = 0; c < 10; c++) { // this loop should last for about 10 x 10 ms
#ifndef WIN32
      usleep(10000); // 10 ms
#else
      Sleep(10);
#endif
      n = serial->read(buffer, 64, false);
      i = 0;
      if (ok == 0) 
        for(;i<n;i++) 
      ...
      if (ok==1) for(;i<n;i++) if (buffer[i]=='K') {
        ok=2;
        break;
      }
    }
    ...
    gUpdateProgressFunction(2, 100-j);
  }
  ...
  while (p) {
    ...
    switch(buffer[0]) {
    case 'K': break;     // all right
    case 'N': return -3; // checksum error
...
int Uploader::send(int robot_id, const std::string &hexfilename, UploaderData::ProgressFunc updateProgressFunction) {
  ...
  if (readfileINHX32(hexfilename)==-1) return -1; // can't open file
